...
macro_rules! screen_enum {
    (pub enum ScreenKind { $($name:ident),*$(,)? }) => {
        ...
        pub trait ScreenMessage: TryFrom<ManagerM, Error = ManagerM> + Into<ManagerM> + 'static {
            fn name() -> &'static str;
        }
        ...
        $(
            ...
            impl TryFrom<ManagerM> for <$name as Screen>::Message {
                type Error = ManagerM;
                fn try_from(msg: ManagerM) -> Result<Self, Self::Error> {
                    match msg {
                        M::$name(msg) => Ok(msg),
                        other => Err(other),
                    }
            ...
            impl ScreenMessage for <$name as Screen>::Message {
                fn name() -> &'static str {
                    stringify!($name)
                }
            }

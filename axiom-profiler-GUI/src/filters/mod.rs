...
use crate::results::filters::{
    CDCL_DISABLER_CHAIN, ONLY_PROOF_STEPS_DISABLER_CHAIN, PROOF_STEPS_DISABLER_CHAIN,
};
...
impl Component for FiltersState {
        ...
        match msg {
            ...
            Msg::SwitchViewerMode(viewer_mode) => {
                ...
                match viewer_mode {
                    ...
                    ViewerMode::CDCL => self.disabler_chain = CDCL_DISABLER_CHAIN.to_vec(),
                }
                self.reset_disabled(&ctx.props().file);
                true
            }
    ...
    fn view(&self, ctx: &Context<Self>) -> Html {
        ...
        let view = match ctx.link().get_state().unwrap().state.viewer_mode {
            ViewerMode::QuantifierInstantiations
            | ViewerMode::ProofSteps
            | ViewerMode::OnlyProofSteps
            | ViewerMode::CDCL => html! {
                <>

...
(assert  (and (= (Ctor PredicateType_validType) 11) (= (type valid) (FieldType PredicateType_validType FrameTypeType))))
(assert (IsPredicateField valid))
(assert (= (getPredWandId valid) 0))
...
(assert (forall ((Heap@@7 T@U) ) (!  (=> (= (type Heap@@7) (MapType0Type RefType)) (|valid#everUsed| valid))
 :qid |stdinbpl.188:15|
 :skolemid |22|
 :pattern ( (|valid#trigger| Heap@@7 valid))
)))
...
(assert (= (type |valid#sm|) (FieldType PredicateType_validType (MapType1Type RefType boolType))))
(assert (= (PredicateMaskField valid) |valid#sm|))

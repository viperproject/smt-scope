(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 20:35:17
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/wands/regression/exec_unfolding.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
(push) ; 2
; [exec]
; package acc(x.f, 1 / 4) && acc(P(x.f), write) --*
...
(check-sat)
; unknown
(assert (P%trigger ($Snap.second $t@2@01) ($SortWrappers.$SnapTo$Ref ($Snap.first $t@2@01))))
(set-option :timeout 0)
(push) ; 4
(set-option :timeout 10)
...
; 0,00s
; (get-info :all-statistics)
; Create MagicWandSnapFunction for wand acc(x.f, 1 / 4) && acc(P(x.f), write) --* acc(x.f, 1 / 4) && acc(x.f.f, 1 / 4)
...
(check-sat)
; unknown
; Create MagicWandSnapFunction for wand acc(x.f, 1 / 4) --* acc(x.f, 1 / 4)
(declare-const mwsf@7@01 $MWSF)
(assert (forall (($t@6@01 $Snap)) (!
  (= (MWSF_apply mwsf@7@01 $t@6@01) $t@6@01)

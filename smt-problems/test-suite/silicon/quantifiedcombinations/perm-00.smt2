...
(push) ; 2
; [exec]
; exhale (forall y: Ref ::false ==> acc(y.f, write))
...
(assert (= ($FVF.lookup_f (as sm@7@01  $FVF<f>) x@1@01) $t@6@01))
...
; [eval] perm(x.f) == write
; [eval] perm(x.f)
(declare-const pm@8@01 $FPM)
(assert (forall ((r $Ref)) (!
  (= ($FVF.perm_f (as pm@8@01  $FPM) r) (ite (= r x@1@01) $Perm.Write $Perm.No))
  :pattern (($FVF.perm_f (as pm@8@01  $FPM) r))
  :qid |qp.resPrmSumDef0|)))
; perm(x.f)  ~~>  assume upper permission bound
(assert (<= ($FVF.perm_f (as pm@8@01  $FPM) x@1@01) $Perm.Write))
(set-option :timeout 0)
(push) ; 3
(assert (not (= ($FVF.perm_f (as pm@8@01  $FPM) x@1@01) $Perm.Write)))
(check-sat)
; unsat
(pop) ; 3
; 0,00s
...
(assert (= ($FVF.perm_f (as pm@8@01  $FPM) x@1@01) $Perm.Write))
; [exec]
; exhale acc(x.f, write) && perm(x.f) == none
; Precomputing data for removing quantified permissions
...
(assert (=
  ...
  ($FVF.lookup_f (as sm@7@01  $FVF<f>) x@1@01)))
; [eval] perm(x.f) == none
; [eval] perm(x.f)
(declare-const pm@11@01 $FPM)
(assert (forall ((r $Ref)) (!
  (= ($FVF.perm_f (as pm@11@01  $FPM) r) $Perm.No)
  :pattern (($FVF.perm_f (as pm@11@01  $FPM) r))
  :qid |qp.resPrmSumDef2|)))
; perm(x.f)  ~~>  assume upper permission bound
(assert (<= ($FVF.perm_f (as pm@11@01  $FPM) x@1@01) $Perm.Write))
(set-option :timeout 0)
(push) ; 3
(assert (not (= ($FVF.perm_f (as pm@11@01  $FPM) x@1@01) $Perm.No)))
(check-sat)
; unsat
(pop) ; 3
; 0,00s
; (get-info :all-statistics)
(assert (= ($FVF.perm_f (as pm@11@01  $FPM) x@1@01) $Perm.No))
(pop) ; 2
(pop) ; 1
; ---------- test02 ----------
...
(push) ; 2
; [exec]
; exhale (forall y: Ref ::false ==> acc(P(y), write))
...
; Definitional axioms for singleton-SM's value
(assert (=
  ($PSF.lookup_P (as sm@20@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01))
  $t@19@01))
; State saturation: after inhale
(set-option :timeout 20)
(check-sat)
...
(define-fun pTaken@21@01 ((x $Ref)) $Perm
  (ite
    (= x x@13@01)
    ($Perm.min (ite (= x x@13@01) $Perm.Write $Perm.No) $Perm.Write)
    $Perm.No))
; Done precomputing, updating quantified chunks
; State saturation: before repetition
(set-option :timeout 10)
...
(assert (not (forall ((x $Ref)) (!
  (=> (= x x@13@01) (= (- $Perm.Write (pTaken@21@01 x)) $Perm.No))
...
; Definitional axioms for snapshot map values (instantiated)
(assert (and
  (not (= ($SortWrappers.$RefTo$Snap x@13@01) $Snap.unit))
  (=
    ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01))
    ($PSF.lookup_P (as sm@20@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01)))))
(assert (=
  ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01))
  ($Snap.combine
    ($Snap.first ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01)))
    ($Snap.second ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01))))))
...
; [eval] x.f > 0
(assert (>
  ($SortWrappers.$SnapToInt ($Snap.first ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01))))
  0))
; State saturation: after unfold
(set-option :timeout 40)
(check-sat)
; unknown
(assert (P%trigger ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01)) x@13@01))
; [exec]
; assert perm(x.f) == write
; [eval] perm(x.f) == write
; [eval] perm(x.f)
...
(assert (forall ((x $Ref)) (!
  (= ($PSF.perm_P (as pm@24@01  $PPM) ($SortWrappers.$RefTo$Snap x)) $Perm.No)
  :pattern (($PSF.perm_P (as pm@24@01  $PPM) ($SortWrappers.$RefTo$Snap x)))
  :qid |qp.resPrmSumDef4|)))
(set-option :timeout 0)
(push) ; 3
(assert (not (=
  ($PSF.perm_P (as pm@24@01  $PPM) ($SortWrappers.$RefTo$Snap x@13@01))
  $Perm.No)))
(check-sat)
; unsat
(pop) ; 3
...
; Definitional axioms for singleton-SM's value
(assert (=
  ($PSF.lookup_P (as sm@25@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01))
  ($Snap.combine
    ($Snap.first ($PSF.lookup_P (as sm@22@01  $PSF<P>) ($SortWrappers.$RefTo$Snap x@13@01)))
    $Snap.unit)))
; [exec]

(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 19:49:45
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/all/issues/carbon/0406.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
(assert (not (= y@0@01 $Ref.null)))
(push) ; 3
(set-option :timeout 10)
(assert (not (= y@0@01 z@1@01)))
(check-sat)
; unknown
(pop) ; 3
; 0,00s
...
;   (forperm x: Ref [x.f] :: x != z ==> 0 / 0 == 0 / 0)
; [eval] (forperm x: Ref [x.f] :: x != z ==> 0 / 0 == 0 / 0)
(set-option :timeout 0)
(push) ; 3
(push) ; 4
(set-option :timeout 10)
...
; unknown
; [exec]
; exhale acc(x.f, write) && (perm(x.f) == none ==> 0 / 0 == 0 / 0)
; [eval] perm(x.f) == none ==> 0 / 0 == 0 / 0
; [eval] perm(x.f) == none
; [eval] perm(x.f)
(set-option :timeout 0)
(push) ; 3

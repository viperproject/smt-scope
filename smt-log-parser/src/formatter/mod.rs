...
pub const DEFAULT_FORMATTER: FormatterConst<'static> = unwrap!(FormatterConst::parse("$-8$${0}$$(#0:-1|4|8|4$(|, |))$$-4$"));
...
impl TermDisplayContext {
    pub fn basic() -> Self {
        let self_: Result<Self, _> = [TRIGGER, UNARY_OP, NEG, BINARY_OP, IF].into_iter().map(|td|
            TermDisplay::try_from(td).unwrap()
        ).collect();
        self_.unwrap()
    }
}
...
impl Default for Formatter {
    fn default() -> Self {
        ...
        FMT.get_or_init(|| Formatter::try_from(DEFAULT_FORMATTER).unwrap()).clone()
    }
}

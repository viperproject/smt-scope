...
(check-sat)
; unknown
; [eval] (unfolding acc(PseudoListinv$(this$_2), wildcard) in (index == 0 ? true : PseudoListget$(this$_2.PseudoListnext$, index - 1)))
...
; 0,00s
; (get-info :all-statistics)
(assert (PseudoListinv$%trigger ($Snap.second s@$) this$_2@0@00))
(assert (=
  ($Snap.second s@$)
  ($Snap.combine
...
(assert (=> (< $Perm.No $k@4@00) (not (= this$_2@0@00 $Ref.null))))
(assert (=
  ($Snap.second ($Snap.second s@$))
  ($Snap.combine
...
; [eval] this$_3.PseudoListnext$ != null
(set-option :timeout 0)
(push) ; 3
(set-option :timeout 10)
(assert (not (< $Perm.No $k@4@00)))
...
; 0,00s
; (get-info :all-statistics)
; [eval] (index == 0 ? true : PseudoListget$(this$_2.PseudoListnext$, index - 1))
; [eval] index == 0
(set-option :timeout 0)
(push) ; 3
(push) ; 4
(set-option :timeout 10)
(assert (not (not (= index@1@00 0))))
(check-sat)
; unknown
(pop) ; 4
; 0,00s
...
(push) ; 4
(set-option :timeout 10)
(assert (not (= index@1@00 0)))
(check-sat)
; unknown
(pop) ; 4
; 0,00s
; (get-info :all-statistics)
; [then-branch: 0 | index@1@00 == 0 | live]
; [else-branch: 0 | index@1@00 != 0 | live]
(set-option :timeout 0)
(push) ; 4
; [then-branch: 0 | index@1@00 == 0]
(assert (= index@1@00 0))
(pop) ; 4
(push) ; 4
; [else-branch: 0 | index@1@00 != 0]
(assert (not (= index@1@00 0)))
; [eval] PseudoListget$(this$_2.PseudoListnext$, index - 1)
(push) ; 5
(set-option :timeout 10)
(assert (not (< $Perm.No $k@4@00)))
(check-sat)
...
(assert (=>
  ...
  (and
    ...
    (PseudoListget$%precondition ($Snap.combine
      ...
      index@1@00
      1)))))
(assert (or (not (= index@1@00 0)) (= index@1@00 0)))
(pop) ; 2
; Joined path conditions
(assert ($Perm.isReadVar $k@5@00))
(assert ($Perm.isReadVar $k@6@00))
(assert (and
  ...
  (PseudoListinv$%trigger ($Snap.second s@$) this$_2@0@00)
  (=
    ($Snap.second s@$)
    ($Snap.combine
      ($Snap.first ($Snap.second s@$))
  ...
  (<= $Perm.No $k@4@00)
  (<= $k@4@00 $Perm.Write)
  (=> (< $Perm.No $k@4@00) (not (= this$_2@0@00 $Ref.null)))
  (=
    ($Snap.second ($Snap.second s@$))
    ($Snap.combine
      ($Snap.first ($Snap.second ($Snap.second s@$)))
  ...
  (=>
    ...
    (and
      ...
      (PseudoListget$%precondition ($Snap.combine
        ...
        index@1@00
        1))))
  (or (not (= index@1@00 0)) (= index@1@00 0))))
(assert (=
  result@2@00
  (ite
    (= index@1@00 0)

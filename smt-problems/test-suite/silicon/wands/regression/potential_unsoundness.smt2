(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 20:37:02
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/wands/regression/potential_unsoundness.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
; [eval] x.f == 2
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@6@01)) 2))
(push) ; 4
(set-option :timeout 10)
(assert (not false))
(check-sat)
...
; 0,00s
; (get-info :all-statistics)
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@6@01)) $t@4@01))
(check-sat)
; unknown
(set-option :timeout 0)
(push) ; 4
...
; 0,00s
; (get-info :all-statistics)
; Create MagicWandSnapFunction for wand acc(x.f, 1 / 2) && x.f == 2 --* acc(x.f, 3 / 4) && acc(y.f, 1 / 2)
...
(assert (forall (($t@6@01 $Snap)) (!
  (and
    (= ($SortWrappers.$SnapToInt ($Snap.first $t@6@01)) $t@4@01)
    (=
      (MWSF_apply mwsf@7@01 $t@6@01)
      ($Snap.combine
        ($SortWrappers.IntTo$Snap $t@4@01)
...
; 0,00s
; (get-info :all-statistics)
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@15@01)) $t@13@01))
(check-sat)
; unknown
(set-option :timeout 0)
(push) ; 4
...
(assert (forall (($t@15@01 $Snap)) (!
  (and
    (= ($SortWrappers.$SnapToInt ($Snap.first $t@15@01)) $t@13@01)
    (=
      (MWSF_apply mwsf@16@01 $t@15@01)
      ($Snap.combine
        ($SortWrappers.IntTo$Snap $t@13@01)
...
(push) ; 4
(set-option :timeout 10)
(assert (not (not (= $t@13@01 $t@14@01))))
(check-sat)
; unknown
(pop) ; 4
; 0,00s
...
(push) ; 4
(set-option :timeout 10)
(assert (not (not (= $t@14@01 $t@13@01))))
(check-sat)
; unknown
(pop) ; 4
; 0,00s

(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 20:36:10
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/wands/regression/known_folded_1.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
(push) ; 2
; [exec]
; package acc(x.f, write) && x.f == 2 --* acc(x.f, write) && x.f == 2 {
;   fold acc(P(x), write)
...
(check-sat)
; unknown
(assert (P%trigger ($Snap.first $t@2@01) x@1@01))
; [exec]
; unfold acc(P(x), write)
(set-option :timeout 0)
(push) ; 4
...
; unknown
; [eval] x.f == 2
; Create MagicWandSnapFunction for wand acc(x.f, write) && x.f == 2 --* acc(x.f, write) && x.f == 2
...
(check-sat)
; unknown
(assert (Q%trigger $t@7@01 x@5@01))
; [exec]
; fold acc(Q(x), write)
(set-option :timeout 0)
(push) ; 5
...
(check-sat)
; unknown
(assert (Q%trigger $t@6@01 x@5@01))
...
(check-sat)
; unknown
; Create MagicWandSnapFunction for wand acc(x.f, 1 / 2) --* acc(Q(x), 2 / 1)
(declare-const mwsf@8@01 $MWSF)
(assert (forall (($t@7@01 $Snap)) (!
  (= (MWSF_apply mwsf@8@01 $t@7@01) $t@7@01)
...
(assert (forall (($t@7@01 $Snap)) (!
  ...
  :qid |quant-u-3|)))
(assert true)
; Create MagicWandSnapFunction for wand acc(x.f, 1 / 2) --* true
(declare-const mwsf@9@01 $MWSF)
(assert (forall (($t@6@01 $Snap)) (!
  (= (MWSF_apply mwsf@9@01 $t@6@01) $Snap.unit)

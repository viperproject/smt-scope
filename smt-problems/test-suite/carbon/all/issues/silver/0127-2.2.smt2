...
(let ((anon3_Then_correct  (=> (not (= FullPerm NoPerm)) (and (=> (= (ControlFlow 0 2) (- 0 3)) (<= FullPerm (U_2_real (MapType1Select ZeroMask null P)))) (=> (<= FullPerm (U_2_real (MapType1Select ZeroMask null P))) (=> (= (ControlFlow 0 2) 1) anon2_correct))))))
(let ((anon0_correct  (=> (state Heap@@10 ZeroMask) (=> (and (|P#trigger| Heap@@10 P) (= (MapType0Select Heap@@10 null P) EmptyFrame)) (and (=> (= (ControlFlow 0 5) 2) anon3_Then_correct) (=> (= (ControlFlow 0 5) 4) anon3_Else_correct))))))

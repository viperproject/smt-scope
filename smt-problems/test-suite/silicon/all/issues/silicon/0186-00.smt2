(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 19:52:20
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/all/issues/silicon/0186.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
(assert (= x@4@01 ($SortWrappers.$SnapTo$Ref ($Snap.first $t@2@01))))
; [exec]
...
(push) ; 4
(set-option :timeout 10)
(assert (not (= l@1@01 ($SortWrappers.$SnapTo$Ref ($Snap.first $t@5@01)))))
(check-sat)
; unknown
(pop) ; 4
; 0,00s
...
(push) ; 4
(set-option :timeout 10)
(assert (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@5@01)) x@4@01)))
(check-sat)
; unknown
(pop) ; 4
; 0,00s
...
(push) ; 4
(set-option :timeout 10)
(assert (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@2@01)) x@4@01)))
(check-sat)
; unsat
(pop) ; 4
; 0,00s
...
(check-sat)
; unknown
(assert (P%trigger ($Snap.second $t@2@01) x@4@01))
(set-option :timeout 0)
(push) ; 4
(set-option :timeout 10)
(assert (not false))
...
(check-sat)
; unknown
; Create MagicWandSnapFunction for wand acc(l.next, 1 / 2) && acc(l.next.next, 1 / 2) --* acc(P(x), write) && acc(l.next, write)
(declare-const mwsf@6@01 $MWSF)
(assert (forall (($t@5@01 $Snap)) (!
  (=
    (MWSF_apply mwsf@6@01 $t@5@01)
    ($Snap.combine ($Snap.second $t@2@01) ($Snap.first $t@2@01)))

...
impl<const FORWARD: bool> DepthInitialiser<FORWARD> for DefaultDepth<FORWARD> {
    fn collect<'n, T: Iterator<Item = &'n Node>>(&mut self, node: &Node, from_all: impl Fn() -> T) -> Depth {
        ...
        // we calculate a 'true' min and max depth from instantiation nodes only.
        let depth_increase = (!is_disabled && node.kind().inst().is_some()) as u32;
        let min = from_all().map(|n| depth(n).min).filter(|min| *min != 0).min().unwrap_or(0) + depth_increase;
        let max = from_all().map(|n| depth(n).max).filter(|max| *max != 0).max().unwrap_or(0) + depth_increase;
        Depth { min, max }
    }
}

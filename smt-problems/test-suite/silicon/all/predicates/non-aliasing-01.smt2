...
(assert (<= $k@2@00 $Perm.Write))
(assert (=> (< $Perm.No $k@2@00) (not (= this@0@00 $Ref.null))))
; [eval] this.next != null
(push) ; 2
(set-option :timeout 10)
(assert (not (< $Perm.No $k@2@00)))
(check-sat)
; unsat
(pop) ; 2
; 0,00s
...
; 0,00s
; (get-info :all-statistics)
; [then-branch: 1 | First:($t@5@00) != Null | live]
; [else-branch: 1 | First:($t@5@00) == Null | live]
(set-option :timeout 0)
(push) ; 2
; [then-branch: 1 | First:($t@5@00) != Null]
(assert (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@5@00)) $Ref.null)))
(pop) ; 2
(push) ; 2
; [else-branch: 1 | First:($t@5@00) == Null]

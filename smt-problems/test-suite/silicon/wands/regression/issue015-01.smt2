(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 20:35:56
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/wands/regression/issue015.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
; 0,00s
; (get-info :all-statistics)
; [then-branch: 0 | First:($t@3@00) != Null | live]
; [else-branch: 0 | First:($t@3@00) == Null | live]
(set-option :timeout 0)
(push) ; 3
; [then-branch: 0 | First:($t@3@00) != Null]
(assert (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@3@00)) $Ref.null)))
(pop) ; 3
(push) ; 3
; [else-branch: 0 | First:($t@3@00) == Null]
...
; Joined path conditions
(assert (or
  (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@3@00)) $Ref.null)
  (not (= ($SortWrappers.$SnapTo$Ref ($Snap.first $t@3@00)) $Ref.null))))
(push) ; 2
(assert (not (or
  (=
    (ite

...
use mem_dbg::*;
use smt_log_parser::{parsers::z3::graph::{raw::{InstEdgeKind, InstNodeKind}, Graph}, LogParser, Z3Parser};
...
fn parse_all_logs() {
    ...
    for log in all_logs {
        ...
        let t = std::thread::spawn(move || {
            ...
            ALLOCATOR.set_limit(mem_limit as usize).unwrap();
            let inst_graph = Graph::<InstNodeKind, InstEdgeKind>::new(parser.parser()).unwrap();
            let elapsed = now.elapsed();

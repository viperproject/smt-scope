(get-info :version)
; (:version "4.8.7")
; Started: 2024-11-17 20:34:47
; Silicon.version: 1.1-SNAPSHOT (4d756c79+)
; Input file: /silicon/silver/src/test/resources/wands/regression/apply1.vpr
; Verifier id: 00
; ------------------------------------------------------------
; Begin preamble
; ////////// Static preamble
...
; [eval] x.f == 2
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@7@01)) 2))
; State saturation: after inhale
(set-option :timeout 20)
(check-sat)
; unknown
...
; [eval] x.f == 2
(assert (= ($SortWrappers.$SnapToInt ($Snap.first $t@8@01)) 2))
(push) ; 4
(set-option :timeout 10)
(assert (not false))
(check-sat)
...
(check-sat)
; unknown
; Create MagicWandSnapFunction for wand acc(x.f, 1 / 4) && x.f == 2 --* acc(x.f, 1 / 4)
(declare-const mwsf@9@01 $MWSF)
(assert (forall (($t@8@01 $Snap)) (!
  (= (MWSF_apply mwsf@9@01 $t@8@01) ($Snap.first $t@8@01))

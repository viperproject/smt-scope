...
impl DeParseTrait for &'_ RepeatFormatter {
    fn deparse(self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        let middle_sep =
            duplicate_character(&DeParse(&self.middle_sep).to_string(), SEPARATOR_CHARACTER);
        write!(f, "{middle_sep}")?;
        write!(f, "{SEPARATOR_CHARACTER}")?;
        let right_sep =
